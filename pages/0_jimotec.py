...
# Logotyp uppe till vänster om filen finns
logo_file = None
...
# Mapp-ID för mapparna i Google Drive
MAPPAR = {
    ...
    if not service:
        ...
        try:
            ...
            if antal_dokument == 0:
                ...
                st.session_state.senaste_mapp = vald_mapp
                st.success(f"✅ Klart! Läste in text från **{antal_dokument}** dokument.")

        except Exception as e:
            meddelande.empty()
            st.error(f"Kunde inte läsa mappen från Google Drive: {e}")
            st.info("Kontrollera att ditt Service Account är tillagd som Läsare (Viewer) på mappen i Google Drive.")

# Visa samlat resultat
if "inlast_text" in st.session_state and st.session_state.inlast_text:
    st.write("---")
    st.subheader(f"📋 Sammanställd fakta från {st.session_state.get('senaste_mapp', vald_mapp)} ({st.session_state.get('antal_filer', 0)} filer)")
    st.caption("Klicka på kopieringsikonen uppe till höger i textrutan nedan och klistra in här i Gemini:")
    ...
    st.download_button(
        ...
        file_name=f"GoogleDrive_{st.session_state.get('senaste_mapp', 'export').replace(' ', '_')}.txt",
        mime="text/plain"
    )
